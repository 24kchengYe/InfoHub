...
class DailySummarizer:
    ...
    def _group_by_category(items: list) -> dict:
        """Group items by their metadata.category field."""
        groups: dict[str, list] = {}
        for item in items:
            cat = (item.metadata or {}).get("category") or "other"
    ...
    async def generate_summary(
        ...
        if language == "zh":
            header_desc = f"> 从 {total_fetched} 条内容中筛选出 {len(items)} 条重要资讯\n\n"
        else:
            header_desc = f"> {len(items)} important items selected from {total_fetched} total\n\n"
        header = (
        ...
        for item in items:
            d = item.domain or "other"
            domain_groups.setdefault(d, []).append(item)

        # TOC — grouped by domain
        toc_entries = []
        idx = 1
        for domain_key in ["ai", "finance", "other"]:
            d_items = domain_groups.get(domain_key, [])
            if not d_items:
                continue
            d_label = DOMAIN_LABELS.get(domain_key, {}).get(language, domain_key.title())
            ...
            for item in d_items:
                ...
                source = item.metadata.get("feed_name") or item.author or ""
                src_tag = f" `{source}`" if source else ""
                toc_entries.append(f"{idx}. [{t}]({item.url}) \u2b50\ufe0f {score}/10{src_tag}")
                idx += 1
        toc = "\n".join(toc_entries) + "\n\n---\n\n"

        # Body — grouped by domain then category
        body = ""
        item_idx = 1
        for domain_key in ["ai", "finance", "other"]:
            d_items = domain_groups.get(domain_key, [])
            ...
            d_label = DOMAIN_LABELS.get(domain_key, {}).get(language, domain_key.title())
            body += f"# {d_label}\n\n"
            groups = self._group_by_category(d_items)
            for cat_key, cat_items in groups.items():
                cat_label = CATEGORY_LABELS.get(cat_key, {}).get(language, cat_key.title())
                body += f"## {cat_label}\n\n"
                for item in cat_items:
                    body += self._format_item(item, labels, language, item_idx)
                    item_idx += 1
    ...
    def generate_webhook_overview(
        ...
        for i, item in enumerate(items, start=1):
            ...
            source = item.metadata.get("feed_name") or item.author or ""
            src_tag = f" `{source}`" if source else ""
            entries.append(f"{i}. [{title}]({item.url}) \u2b50\ufe0f {score}/10{src_tag}")
