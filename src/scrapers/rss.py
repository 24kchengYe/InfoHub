...
class RSSScraper(BaseScraper):
    ...
    async def _fetch_feed(
        ...
        try:
            ...
            # Fetch feed content — use a no-proxy client for localhost URLs
            if feed_url.startswith("http://localhost") or feed_url.startswith("http://127.0.0.1"):
                async with httpx.AsyncClient(timeout=90.0) as local_client:
                    response = await local_client.get(feed_url, follow_redirects=True)
            else:
            ...
            for entry in feed.entries:
                # Parse published date; treat undated entries as fresh
